...
function _relatedlinks(rlinkmsgid) {
	if(!G_$_D(rlinkmsgid) || G_$_D(rlinkmsgid).innerHTML.match(/<script[^\>]*?>/i)) {
		return;
	}
	var alink = new Array(), ignore = new Array();
	var i = 0;
	var msg = G_$_D(rlinkmsgid).innerHTML;
	...
	G_$_D(rlinkmsgid).innerHTML = msg;
}

function _updatesecqaa(idhash) {
	if(G_$_D('secqaa_' + idhash)) {
		G_$_D('secqaaverify_' + idhash).value = '';
		if(secST['qaa_' + idhash]) {
			clearTimeout(secST['qaa_' + idhash]);
		}
		G_$_D('checksecqaaverify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/none.gif" width="16" height="16" class="vm" />';
		ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function() {
			secST['qaa_' + idhash] = setTimeout(function() {G_$_D('secqaa_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updatesecqaa(\''+idhash+'\')">刷新验证问答</span>';}, 180000);
		});
	}
}
...
function _updateseccode(idhash, play) {
	if(isUndefined(play)) {
		if(G_$_D('seccode_' + idhash)) {
			G_$_D('seccodeverify_' + idhash).value = '';
			if(secST['code_' + idhash]) {
				clearTimeout(secST['code_' + idhash]);
			}
			G_$_D('checkseccodeverify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/none.gif" width="16" height="16" class="vm" />';
			ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function() {
				secST['code_' + idhash] = setTimeout(function() {G_$_D('seccode_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updateseccode(\''+idhash+'\')">刷新验证码</span>';}, 180000);
			});
		}
	} else {
...
function _checksec(type, idhash, showmsg, recall) {
	...
	var secverify = G_$_D('sec' + type + 'verify_' + idhash).value;
	if(!secverify) {
		return;
	}
	var x = new Ajax('XML', 'checksec' + type + 'verify_' + idhash);
	x.loading = '';
	G_$_D('checksec' + type + 'verify_' + idhash).innerHTML = '<img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" />';
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		var obj = G_$_D('checksec' + type + 'verify_' + idhash);
		obj.style.display = '';
...
function _setDoodle(fid, oid, url, tid, from) {
	if(tid == null) {
		...
		G_$_D(tid).style.display = '';
		G_$_D(fid).style.display = 'none';
	}
...
function _showdistrict(container, elems, totallevel, changelevel, containertype) {
	...
	var pid = changelevel >= 1 && elems[0] && G_$_D(elems[0]) ? getdid(G_$_D(elems[0])) : 0;
	var cid = changelevel >= 2 && elems[1] && G_$_D(elems[1]) ? getdid(G_$_D(elems[1])) : 0;
	var did = changelevel >= 3 && elems[2] && G_$_D(elems[2]) ? getdid(G_$_D(elems[2])) : 0;
	var coid = changelevel >= 4 && elems[3] && G_$_D(elems[3]) ? getdid(G_$_D(elems[3])) : 0;
...
function _showselect(obj, inpid, t, rettype) {
	var showselect_row = function (inpid, s, v, notime, rettype) {
		if(v >= 0) {
			if(!rettype) {
				...
				return '<a href="javascript:;" onclick="G_$_D(\'' + inpid + '\').value = \'' + d.getFullYear() + '-' + month + '-' + day + (!notime ? ' ' + hour + ':' + minute: '') + '\'">' + s + '</a>';
			} else {
				return '<a href="javascript:;" onclick="G_$_D(\'' + inpid + '\').value = \'' + v + '\'">' + s + '</a>';
			}
		} else if(v == -1) {
			return '<a href="javascript:;" onclick="G_$_D(\'' + inpid + '\').focus()">' + s + '</a>';
		} else if(v == -2) {
			return '<a href="javascript:;" onclick="G_$_D(\'' + inpid + '\').onclick()">' + s + '</a>';
		}
	};
	...
	if(!obj.id) {
		...
		div.className = 'p_pop';
		G_$_D('append_parent').appendChild(div);
		s = '';
		if(!t) {
			...
			if(G_$_D(t)) {
				var lis = G_$_D(t).getElementsByTagName('LI');
				for(i = 0;i < lis.length;i++) {
					s += '<a href="javascript:;" onclick="G_$_D(\'' + inpid + '\').value = this.innerHTML;G_$_D(\''+obj.id+'_menu\').style.display=\'none\'">' + lis[i].innerHTML + '</a>';
				}
				s += showselect_row(inpid, '自定义', -1);
			} else {
				s += '<a href="javascript:;" onclick="G_$_D(\'' + inpid + '\').value = \'0\'">永久</a>';
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	var menuid = 'imgzoom';
	var menu = G_$_D(menuid);
	var zoomid = menuid + '_zoom';
	var imgtitle = !nocover && obj.title ? '<div class="imgzoom_title hide">' + obj.title + '</div>' +
	...
	var loadCheck = function (obj) {
		if(obj.complete) {
			...
			var h = w / r;
			if(w < 100 & h < 100) {
				G_$_D(menuid + '_waiting').style.display = 'none';
				hideMenu();
				return;
			}
			if(h > maxh) {
			...
			if(G_$_D(menuid)) {
				G_$_D(menuid).removeAttribute('top_');G_$_D(menuid).removeAttribute('left_');
				clearTimeout(G_$_D(menuid).getAttribute('timer'));
			}
			...
			if(showexif && faid) {
				...
				x.get('forum.php?mod=ajax&action=exif&aid=' + faid + '&inajax=1', function(s, x) {
					if(s) {
						G_$_D(zoomid + '_exif').style.display = '';
						G_$_D(zoomid + '_exif').innerHTML = s;
					} else {
						G_$_D(zoomid + '_exif').style.display = 'none';
					}
				});
			}
		} else {
	...
	var showloading = function (zimg, pn) {
		if(!pn) {
			if(!G_$_D(menuid + '_waiting')) {
				...
				G_$_D('append_parent').appendChild(waiting);
			}
		}
		G_$_D(menuid + '_waiting').style.display = '';
		G_$_D(menuid + '_waiting').style.left = (document.body.clientWidth - 42) / 2 + 'px';
		G_$_D(menuid + '_waiting').style.top = ((document.documentElement.clientHeight - 42) / 2 + Math.max(document.documentElement.scrollTop, document.body.scrollTop)) + 'px';
		loading = new Image();
		setTimeout(function () { loadCheck(loading); }, 100);
		if(!pn) {
			G_$_D(menuid + '_zoomlayer').style.display = 'none';
		}
		loading.src = zimg;
	};
	var adjustpn = function(h) {
		h = h < 90 ? 90 : h;
		if(G_$_D('zimg_prev')) {
			G_$_D('zimg_prev').style.height= parseInt(h) + 'px';
		}
		if(G_$_D('zimg_next')) {
			G_$_D('zimg_next').style.height= parseInt(h) + 'px';
		}
	};
	var showimage = function (zimg, w, h, imgw, imgh) {
		G_$_D(menuid + '_waiting').style.display = 'none';
		G_$_D(menuid + '_zoomlayer').style.display = '';
		G_$_D(menuid + '_img').style.width = 'auto';
		G_$_D(menuid + '_img').style.height = 'auto';
		G_$_D(menuid).style.width = (w < 300 ? 320 : w + 20) + 'px';
		mheight = h + 63;
		menu.style.height = mheight + 'px';
		G_$_D(menuid + '_zoomlayer').style.height = (mheight < 120 ? 120 : mheight) + 'px';
		G_$_D(menuid + '_img').innerHTML = '<img id="' + zoomid + '" src="' + zimg + '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="' + imgh + '" />' + imgtitle;
		if(G_$_D(menuid + '_imglink')) {
			G_$_D(menuid + '_imglink').href = zimg;
		}
		...
		if(BROWSER.ie && BROWSER.ie<7) {
			...
			if(adjustTimerCount) {
				adjustTimer = (function(){
					...
					G_$_D(menuid).setAttribute('timer', adjustTimer);
	...
	if(!menu && !pn) {
		...
		if(cover) {
		    menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none">'+
				'<div class="zimg_p hide" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>'+
            '<p class="zoom_text"><span class="y">' + '<a href="javascript:;" onclick="_ACT_imgupload(this);" class="imgdownload" title="下载"></a>' + '<a id="' + menuid + '_imglink" class="imglink" target="_blank" title="在新窗口打开"></a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="实际大小"></a>' +
            '<a href="javascript:;" onclick="_ACT_imgroate(this);" class="imgroate" title="旋转"></a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="关闭"></a>' +
		...
		G_$_D('append_parent').appendChild(menu);
		if(G_$_D(menuid + '_adjust')) {
			G_$_D(menuid + '_adjust').onclick = function(e) {adjust(e, 1)};
		}
	}
	showloading(zimg, pn);
	picpage = '';
	G_$_D(menuid + '_picpage').innerHTML = '';
	if( false || (typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]])) {
		...
		if(authorlength > 1) {
			...
			if(picpage) {
				G_$_D(menuid + '_picpage').innerHTML = picpage;
			}
		}
	}
...
function _zoom_page(paid, showexif) {
	var imagesrc = G_$_D('aimg_' + paid).getAttribute('zoomfile') ? G_$_D('aimg_' + paid).getAttribute('zoomfile') : G_$_D('aimg_' + paid).getAttribute('file');
	zoom(G_$_D('aimg_' + paid), imagesrc, 0, 1, showexif ? 1 : 0);
}

function _switchTab(prefix, current, total, activeclass) {
	activeclass = !activeclass ? 'a' : activeclass;
	for(var i = 1; i <= total;i++) {
		var classname = ' '+G_$_D(prefix + '_' + i).className+' ';
		G_$_D(prefix + '_' + i).className = classname.replace(' '+activeclass+' ','').substr(1);
		G_$_D(prefix + '_c_' + i).style.display = 'none';
	}
	G_$_D(prefix + '_' + current).className = G_$_D(prefix + '_' + current).className + ' '+activeclass;
	G_$_D(prefix + '_c_' + current).style.display = '';
}

function _initTab(frameId, type) {
	if (typeof document['diyform'] == 'object' || G_$_D(frameId).className.indexOf('tab') < 0) return false;
	type = type || 'click';
	var tabs = G_$_D(frameId+'_title').childNodes;
	var arrTab = [];
	for(var i in tabs) {
	...
	for(var i = 0;i < len; i++) {
		...
		tab.appendChild(li);
		G_$_D(frameId+'_title').removeChild(arrTab[i]);
		counter++;
	}
	G_$_D(frameId+'_title').appendChild(tab);
}
...
function switchTabUl (e) {
	...
	for(var j = 0; j < len2; j++) {
		...
		var content = G_$_D(tabs[j].id+'_content');
...
function slideshow(el) {
	...
	this.checkLoad = function () {
		...
		var percentEle = G_$_D(this.id+'_percent');
...
function _showTip(ctrlobj) {
	...
	G_$_D(ctrlobj.id).onmouseout = function () { hideMenu('', 'prompt'); };
...
function _showPrompt(ctrlid, evt, msg, timeout) {
	...
	if(G_$_D(menuid)) {
		G_$_D(menuid).parentNode.removeChild(G_$_D(menuid));
	}
	var div = document.createElement('div');
	div.id = menuid;
	...
	if(ctrlid) {
		...
		if(G_$_D(ctrlid)) {
			if(G_$_D(ctrlid).evt !== false) {
				var prompting = function() {
					showMenu({'mtype':'prompt','ctrlid':ctrlid,'evt':evt,'menuid':menuid,'pos':'210'});
				};
				if(evt == 'click') {
					G_$_D(ctrlid).onclick = prompting;
				} else {
					G_$_D(ctrlid).onmouseover = prompting;
				}
			}
			showMenu({'mtype':'prompt','ctrlid':ctrlid,'evt':evt,'menuid':menuid,'pos':'210','duration':duration,'timeout':timeout,'zindex':JSMENU['zIndex']['prompt']});
			G_$_D(ctrlid).unselectable = false;
		}
	} else {
		showMenu({'mtype':'prompt','pos':'00','menuid':menuid,'duration':duration,'timeout':timeout,'zindex':JSMENU['zIndex']['prompt']});
		G_$_D(menuid).style.top = (parseInt(G_$_D(menuid).style.top) - 100) + 'px';
	}
}
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
	...
	for(i = 1; i <= 8; i++) {
		...
		if(G_$_D('hcredit_' + i)) {
			G_$_D('hcredit_' + i).innerHTML = basev[i];
		}
	}
	for(i = 1; i <= 8; i++) {
		if(notice[i] != 0) {
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
	...
	if(!G_$_D(ctrlid + '_menu')) {
		...
		for(var i = 0; i < 40; i++) {
			...
			+ (typeof wysiwyg == 'undefined' ? 'seditor_insertunit(\'' + k + '\', \'[' + tag1 + '=' + coloroptions[i] + ']\', \'[/' + tag1 + ']\')' : (ctrlid == editorid + '_tbl_param_4' ? 'G_$_D(\'' + ctrlid + '\').value=\'' + coloroptions[i] + '\';hideMenu(2)' : 'discuzcode(\'' + tag2 + '\', \'' + coloroptions[i] + '\')'))
...
function _extstyle(css) {
	if(!G_$_D('css_extstyle')) {
		loadcss('extstyle');
	}
	G_$_D('css_extstyle').href = css ? css + '/style.css' : STATICURL + 'image/common/extstyle_none.css';
	currentextstyle = css;
	setcookie('extstyle', css, 86400 * 30);
	if(G_$_D('css_widthauto') && !G_$_D('css_widthauto').disabled) {
...
function _widthauto(obj) {
	if(G_$_D('css_widthauto')) {
		CSSLOADED['widthauto'] = 1;
	}
	if(!CSSLOADED['widthauto'] || G_$_D('css_widthauto').disabled) {
		if(!CSSLOADED['widthauto']) {
			loadcss('widthauto');
		} else {
			G_$_D('css_widthauto').disabled = false;
		}
		HTMLNODE.className += ' widthauto';
		setcookie('widthauto', 1, 86400 * 30);
		obj.innerHTML = '切换到窄版';
	} else {
		G_$_D('css_widthauto').disabled = true;
...
function _showCreditmenu() {
	if(!G_$_D('extcreditmenu_menu')) {
		...
		G_$_D('append_parent').appendChild(menu);
		ajaxget(G_$_D('extcreditmenu').href, 'extcreditmenu_menu', 'ajaxwaitid');
	}
...
function _imageRotate(imgid, direct) {
	...
	if(BROWSER.ie) {
		...
		var canvas = G_$_D(image.getAttribute('canvasid'));
		if(!canvas) {
...
function _createPalette(colorid, id, func) {
	...
	if (!G_$_D("c"+colorid+"_menu")) {
		var dom = document.createElement('span');
		dom.id = "c"+colorid+"_menu";
		dom.style.display = 'none';
		dom.innerHTML = iframe;
		G_$_D('append_parent').appendChild(dom);
	}
	func = !func ? '' : '|' + func;
